...
export class ResidencesComponentComponent {
  ...
  listResidences:Residence[]=[
   ...
   show(rs:Residence){
    this.listrs=this.listApartments.filter(el=>el.residence==rs)
    // this.listrs=[];
    // for(let i of this.listApartments){
    //   if(i.residence==rs){
    //     this.listrs.push(i);
    //   }
    // }
   ...
   favorit(appart:Apartment){
    if(this.favoritlist.findIndex(ap=>appart.id == ap.id)==-1){
      this.favoritlist.push(appart);
    }
   }
